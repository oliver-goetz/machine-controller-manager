...
func (c *controller) getBootstrapTokenOrCreateIfNotExist(ctx context.Context, machine *v1alpha1.Machine) (secret *corev1.Secret, err error) {
	...
	if err != nil {
		if apierrors.IsNotFound(err) {
			...
			authExtraGroups := fmt.Sprintf("system:bootstrappers:machine:%s", machine.Name)
			if c.bootstrapTokenAuthExtraGroups != "" {
				authExtraGroups = fmt.Sprintf("%s,%s", authExtraGroups, c.bootstrapTokenAuthExtraGroups)
			}
			data := map[string][]byte{
				...
				bootstraptokenapi.BootstrapTokenExtraGroupsKey:      []byte(authExtraGroups),
			}
